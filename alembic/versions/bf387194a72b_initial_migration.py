...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE tipousuario AS ENUM ('admin', 'consultor')")
    op.execute("CREATE TYPE statusprojeto AS ENUM ('planejado', 'em_andamento', 'concluido', 'cancelado', 'pausado')")
    op.execute("CREATE TYPE tiponotificacao AS ENUM ('PROSPECCAO_CRIADA', 'AGENDAMENTO_CRIADO', 'AGENDAMENTO_REALIZADO', 'EMPRESA_ATRIBUIDA', 'PROSPECCAO_ATUALIZADA')")
    op.execute("CREATE TYPE statusagendamento AS ENUM ('pendente', 'realizado', 'vencido')")
    op.execute("CREATE TYPE statusatividade AS ENUM ('pendente', 'em_andamento', 'concluida', 'cancelada')")
